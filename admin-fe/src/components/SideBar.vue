...
<template>
  <div
    @click="show"
    class="fixed left-0 sm:hidden p-3"
    :class="{ hide: !isHidden }"
  >
    <svg
      class="block h-6 w-6"
  ...
  <div
    class="h-screen left-0 top-0 bottom-0 p-2 w-[300px] overflow-y-auto bg-gray-900 fixed sm:relative"
  ...
  >
    <div class="flex items-center justify-center mt-3">
      ...
      <span
        class="btn-close hover:bg-blue-600 rounded-md cursor-pointer flex sm:hidden"
        @click="close"
        >×</span
      >
    </div>
    ...
    <div class="text-left text-sm pl-4 mt-2 mx-auto text-gray-200 font-bold">
      ...
      <router-link
        :to="{ name: routes.NOTE_CREATE }"
        class="py-2.5 px-4 hover:bg-blue-600 rounded-md mt-1 block text-white"
      >
        {{ $t('create') }}
      </router-link>
      <div
        class="cursor-pointer py-2.5 px-4 hover:bg-blue-600 rounded-md mt-1 block"
        @click="publishAllNotes"
      >
        {{ $t('rePublishAll') }}
      </div>
    </div>
    <div class="my-4 bg-gray-600 h-[1px]"></div>
    <router-link
      :to="{ name: routes.TODAY_I_LEARNEDS }"
      class="py-2.5 mt-3 flex items-center rounded-md px-4 duration-300 cursor-pointer hover:bg-blue-600 text-white"
    >
    ...
    <div class="text-left text-sm pl-4 mt-2 mx-auto text-gray-200 font-bold">
      ...
      <router-link
        :to="{ name: routes.TODAY_I_LEARNED_CREATE }"
        class="py-2.5 px-4 hover:bg-blue-600 rounded-md mt-1 block text-white"
      >
        {{ $t('create') }}
      </router-link>
      <div
        class="cursor-pointer py-2.5 px-4 hover:bg-blue-600 rounded-md mt-1 block"
        @click="publishAllTils"
      >
        {{ $t('rePublishAll') }}
      </div>
      <div class="my-4 bg-gray-600 h-[1px]"></div>
      <router-link
        :to="{ name: routes.EMOJIS }"
        class="py-2.5 mt-3 flex items-center rounded-md px-4 duration-300 cursor-pointer hover:bg-blue-600 text-white"
      >
        <span class="ml-1 text-gray-200 font-bold">{{ $t('emoji') }}</span>
      </router-link>
      <div class="text-left text-sm pl-4 mt-2 mx-auto text-gray-200 font-bold">
        <router-link
          :to="{ name: routes.EMOJIS }"
